...
import { IonHeader, IonContent, IonList, IonInfiniteScroll, IonInfiniteScrollContent, InfiniteScrollCustomEvent, IonItem, IonSearchbar, ModalController } from "@ionic/angular/standalone";
...
@Component({
  ...
  standalone: true,
  imports: [IonSearchbar, IonItem, IonList, IonContent, DatePipe, DecimalPipe, IonHeader, HeaderBarComponent, IonInfiniteScroll, IonInfiniteScrollContent]
})
export class SellsListComponent  implements OnInit {
  ...
  constructor(private _purchases: LocalPurchaseService, private _modalCtrl: ModalController) { }
  ...
  private generateItems(purchases: Array<IPurchase>, offset: number = 25){
  ...
  protected onSearch($event: CustomEvent){
    ...
    const newList = this.sells.filter(sell => {
      ...
      return (
        ...
        new Date(sell.date).toISOString().trim().toLowerCase().includes(value) ||
